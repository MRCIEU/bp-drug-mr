...
#Read in input data
phen_bi <- fread(paste0("/user/work/ac14629/MRC_network_project/data/UKB/phenotype/mrc_phenotypes_sas.txt"), header=T, fill = T, na.strings = c("", NA), data.table = F, stringsAsFactors = FALSE)
...
#Save output files 
write.table(assoc_out_sorted, "/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/SAS/GENESIS_assoc_chd_out.txt", row.names=F, quote=F)

#Create Manhattan and Q-Q plot 
png(file= "/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/SAS/GENESIS_chd_assoc_manhattan.png", width=8, height=7, units="in", res=300)
manhattan(assoc_out_sorted, chr="chr", bp="pos", snp="variant.id", p="SPA.pval" )
dev.off()

png(file= "/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/SAS/GENESIS_chd_assoc_qq.png", width=8, height=7, units="in", res=300)
