...
snpgdsBED2GDS(bed.fn = "/user/work/ac14629/MRC_network_project/data/UKB/plink/EAS/merged_chr1_22/chr1-22_merged.bed",
              bim.fn = "/user/work/ac14629/MRC_network_project/data/UKB/plink/EAS/merged_chr1_22/chr1-22_merged.bim",
              fam.fn = "/user/work/ac14629/MRC_network_project/data/UKB/plink/EAS/merged_chr1_22/chr1-22_merged.fam",
              out.gdsfn = "/user/work/ac14629/MRC_network_project/data/UKB/plink/EAS/merged_chr1_22/EAS.genotype.gds")
...
# LD pruning
genoData <- snpgdsOpen(filename = paste0(dir, "/EAS/merged_chr1_22/EAS.genotype.gds"))
...
write.table(KINGmat.pair, "/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/EAS/kinship_KING_EAS.txt",quote=F, row.names=F)

pdf("/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/EAS/KING_IBS0_EAS.pdf")
...
geno <- GdsGenotypeReader(filename = paste0(dir, "/EAS/merged_chr1_22/EAS.genotype.gds"))
genoData <- GenotypeData(geno)
...
write.table(varExp,"/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/EAS/VarianceExplainedNumber.txt", quote=F)

pdf("/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/EAS/VarianceExplained_EAS.pdf")
#screeplot(mypcair)
plot(mypcair$values/sum(mypcair$values) * 100, xlab="PCs",ylab="Variance Explained (%)")
dev.off()

#plot PC-AiR PCs 
png(file= paste0(diro, "/EAS/GENESIS_pcair_pc1_pc2.png"), width=8, height=7, units="in", res=300)
plot(mypcair)
dev.off() 
png(file= paste0(diro, "/EAS/GENESIS_pcair_pc3_pc4.png"), width=8, height=7, units="in", res=300)
plot(mypcair, vx = 3, vy = 4)
dev.off() 
png(file= paste0(diro, "/EAS/GENESIS_pcair_pc5_pc6.png"), width=8, height=7, units="in", res=300)
plot(mypcair, vx = 5, vy = 6)
dev.off() 
png(file= paste0(diro, "/EAS/GENESIS_pcair_pc7_pc8.png"), width=8, height=7, units="in", res=300)
plot(mypcair, vx = 7, vy = 8)
dev.off() 
png(file= paste0(diro, "/EAS/GENESIS_pcair_pc9_pc10.png"), width=8, height=7, units="in", res=300)
plot(mypcair, vx = 9, vy = 10)
dev.off() 

pca.df <- data.frame(mypcair$vectors)
colnames(pca.df) <- c(paste("PC",1:200,sep=""))
write.table(pca.df, "/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/EAS/EigenVectors_EAS.txt",sep="\t",quote=FALSE)

# Write table of related set
related_set = mypcair$rels
write.table(related_set, "/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/EAS/Related_King_EAS.txt", quote=F)

#Write table of unrelated set 
unrelated_set = mypcair$unrels
write.table(unrelated_set, "/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/EAS/Unrelated_King_EAS.txt", quote=F)

#save the PCA plots
pdf("/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/EAS/PC_Air_EAS.pdf")
plot(mypcair)
par(mfrow=c(2,1))
plot(mypcair, vx=3, vy=4)
dev.off()
...
save(mypcrelate, file ="/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/EAS/pcrelate_kinship_EAS.RData")
write.table(mypcrelate$kinBtwn,"/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/EAS/pcrelate_kinship_EAS.txt",row.names=F,col.names=T,sep="\t",quote=F)

# plot kinship vs k0 
pdf("/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/EAS/pc-relate_IBS0_EAS_round1.pdf")
...
kinship.scaled.file <- paste("kinshipScaled_analysis_EAS_results_all_",npca,"PC.txt",sep="")
kinship.x<- kinship_scaled@x
write.table(kinship.x,file="/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/EAS/kinship.scaled.EAS.file",row.names=F,col.names=T,sep="\t",quote=F)
