...
snpgdsBED2GDS(bed.fn = paste0(dir, "/AFR/merged_chr1_22/chr1-22_merged.bed"),
              bim.fn = paste0(dir, "/AFR/merged_chr1_22/chr1-22_merged.bim"),
              fam.fn = paste0(dir, "/AFR/merged_chr1_22/chr1-22_merged.fam"),
              out.gdsfn = paste0(dir, "/AFR/merged_chr1_22/AFR.genotype.gds"))
...
# LD pruning
genoData <- snpgdsOpen(filename = paste0(dir, "/AFR/merged_chr1_22/AFR.genotype.gds"))
...
write.table(KINGmat.pair, "/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/AFR/kinship_KING_AFR.txt",quote=F, row.names=F)

pdf("/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/AFR/KING_IBS0_AFR.pdf")
...
write.table(varExp,"/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/AFR/VarianceExplainedNumber.txt", quote=F)

pdf("/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/AFR/VarianceExplained_AFR.pdf")
#screeplot(mypcair)
plot(mypcair$values/sum(mypcair$values) * 100, xlab="PCs",ylab="Variance Explained (%)")
dev.off()

#plot PC-AiR PCs 
png(file= paste0(diro, "/AFR/GENESIS_pcair_pc1_pc2.png"), width=8, height=7, units="in", res=300)
plot(mypcair)
dev.off() 
png(file= paste0(diro, "/AFR/GENESIS_pcair_pc3_pc4.png"), width=8, height=7, units="in", res=300)
plot(mypcair, vx = 3, vy = 4)
dev.off() 
png(file= paste0(diro, "/AFR/GENESIS_pcair_pc5_pc6.png"), width=8, height=7, units="in", res=300)
plot(mypcair, vx = 5, vy = 6)
dev.off() 
png(file= paste0(diro, "/AFR/GENESIS_pcair_pc7_pc8.png"), width=8, height=7, units="in", res=300)
plot(mypcair, vx = 7, vy = 8)
dev.off() 
png(file= paste0(diro, "/AFR/GENESIS_pcair_pc9_pc10.png"), width=8, height=7, units="in", res=300)
plot(mypcair, vx = 9, vy = 10)
dev.off() 

pca.df <- data.frame(mypcair$vectors)
colnames(pca.df) <- c(paste("PC",1:200,sep=""))
write.table(pca.df, "/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/AFR/EigenVectors_AFR.txt",sep="\t",quote=FALSE)

# Write table of related set
related_set = mypcair$rels
write.table(related_set, "/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/AFR/Related_King_AFR.txt", quote=F)

#Write table of unrelated set 
unrelated_set = mypcair$unrels
write.table(unrelated_set, "/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/AFR/Unrelated_King_AFR.txt", quote=F)

#save the PCA plots
pdf("/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/AFR/PC_Air_AFR.pdf")
plot(mypcair)
par(mfrow=c(2,1))
plot(mypcair, vx=3, vy=4)
dev.off()
...
save(mypcrelate, file ="/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/AFR/pcrelate_kinship_AFR.RData")
write.table(mypcrelate$kinBtwn,"/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/AFR/pcrelate_kinship_AFR.txt",row.names=F,col.names=T,sep="\t",quote=F)

# plot kinship vs k0 
pdf("/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/AFR/pc-relate_IBS0_AFR_round1.pdf")
...
kinship.scaled.file <- paste("kinshipScaled_analysis_AFR_results_all_",npca,"PC.txt",sep="")
kinship.x<- kinship_scaled@x
write.table(kinship.x,file="/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/AFR/kinship.scaled.AFR.file",row.names=F,col.names=T,sep="\t",quote=F)
