...
dir <- c("/user/work/ac14629/MRC_network_project/data/UGR/Genetic")
diro <- c("/user/work/ac14629/MRC_network_project/results/UGR/GENESIS") #e.g. "your-home-directory/bp-drug-mr/outputs"

##Convert plink genotype file to GDS files

snpgdsBED2GDS(bed.fn = "/user/work/ac14629/MRC_network_project/data/UGR/Genetic/filtered.UGR.bed",
              bim.fn = "/user/work/ac14629/MRC_network_project/data/UGR/Genetic/filtered.UGR.bim",
              fam.fn = "/user/work/ac14629/MRC_network_project/data/UGR/Genetic/filtered.UGR.fam",
              out.gdsfn = "/user/work/ac14629/MRC_network_project/data/UGR/Genetic/UGR.genotype.gds")
...
# LD pruning
genoData <- snpgdsOpen(filename = paste0(dir, "/UGR.genotype.gds"))
...
write.table(KINGmat.pair, "/user/work/ac14629/MRC_network_project/results/UGR/GENESIS/kinship_KING_UGR.txt",quote=F, row.names=F)

pdf("/user/work/ac14629/MRC_network_project/results/UGR/GENESIS/KING_IBS0_UGR.pdf")
...
geno <- GdsGenotypeReader(filename = paste0(dir, "/UGR.genotype.gds"))
...
write.table(varExp,"/user/work/ac14629/MRC_network_project/results/UGR/GENESIS/VarianceExplainedNumber.txt", quote=F)

pdf("/user/work/ac14629/MRC_network_project/results/UGR/GENESIS/VarianceExplained_UGR.pdf")
#screeplot(mypcair)
plot(mypcair$values/sum(mypcair$values) * 100, xlab="PCs",ylab="Variance Explained (%)")
dev.off()

#plot PC-AiR PCs 
png(file= paste0(diro, "/GENESIS_pcair_pc1_pc2.png"), width=8, height=7, units="in", res=300)
plot(mypcair)
dev.off() 
png(file= paste0(diro, "/GENESIS_pcair_pc3_pc4.png"), width=8, height=7, units="in", res=300)
plot(mypcair, vx = 3, vy = 4)
dev.off() 
png(file= paste0(diro, "/GENESIS_pcair_pc5_pc6.png"), width=8, height=7, units="in", res=300)
plot(mypcair, vx = 5, vy = 6)
dev.off() 
png(file= paste0(diro, "/GENESIS_pcair_pc7_pc8.png"), width=8, height=7, units="in", res=300)
plot(mypcair, vx = 7, vy = 8)
dev.off() 
png(file= paste0(diro, "/GENESIS_pcair_pc9_pc10.png"), width=8, height=7, units="in", res=300)
plot(mypcair, vx = 9, vy = 10)
dev.off() 

pca.df <- data.frame(mypcair$vectors)
colnames(pca.df) <- c(paste("PC",1:200,sep=""))
write.table(pca.df, "/user/work/ac14629/MRC_network_project/results/UGR/GENESIS/EigenVectors_UGR.txt",sep="\t",quote=FALSE)

# Write table of related set
related_set = mypcair$rels
write.table(related_set, "/user/work/ac14629/MRC_network_project/results/UGR/GENESIS/Related_King_UGR.txt", quote=F)

#Write table of unrelated set 
unrelated_set = mypcair$unrels
write.table(unrelated_set, "/user/work/ac14629/MRC_network_project/results/UGR/GENESIS/Unrelated_King_UGR.txt", quote=F)

#save the PCA plots
pdf("/user/work/ac14629/MRC_network_project/results/UGR/GENESIS/PC_Air_UGR.pdf")
plot(mypcair)
par(mfrow=c(2,1))
plot(mypcair, vx=3, vy=4)
dev.off()
...
save(mypcrelate, file ="/user/work/ac14629/MRC_network_project/results/UGR/GENESIS/pcrelate_kinship_UGR.RData")
write.table(mypcrelate$kinBtwn,"/user/work/ac14629/MRC_network_project/results/UGR/GENESIS/pcrelate_kinship_UGR.txt",row.names=F,col.names=T,sep="\t",quote=F)

# plot kinship vs k0 
pdf("/user/work/ac14629/MRC_network_project/results/UGR/GENESIS/pc-relate_IBS0_UGR.pdf")
...
kinship.scaled.file <- paste("kinshipScaled_analysis_UGR_results_all_",npca,"PC.txt",sep="")
kinship.x<- kinship_scaled@x
write.table(kinship.x,file="/user/work/ac14629/MRC_network_project/results/UGR/GENESIS/kinship.scaled.UGR.file",row.names=F,col.names=T,sep="\t",quote=F)
save.image("/user/work/ac14629/MRC_network_project/results/UGR/GENESIS/relatedness.RData")
