...
#Read in input data
phen_con <- read.table("/user/work/ac14629/MRC_network_project/data/UKB/phenotype/mrc_phenotypes_eas_int.txt", header=T, fill = T, na.strings = c("", NA), stringsAsFactors = FALSE)
...
phen_select_con <- phen_con %>% select("IID", "tg") #change phenotype
dat <- merge(phen_select_con, cov_replace, by="IID") %>% merge(., pc, by="IID") 
dat <- dat %>% na.omit()

input_dat <- data.frame(scanID = dat$ieu, pheno = dat$tg, age = dat$age, sex = dat$sex, pc1 = dat$PC1, pc2 = dat$PC2, pc3 = dat$PC3, pc4 = dat$PC4, pc5 = dat$PC5, pc6 = dat$PC6, pc7 = dat$PC7, pc8 = dat$PC8, pc9 = dat$PC9, pc10= dat$PC10, pc11 = dat$PC11, pc12 = dat$PC12, pc13 = dat$PC13, pc14 = dat$PC14, pc15 = dat$PC15, pc16 = dat$PC16, pc17 = dat$PC17, pc18 = dat$PC18, pc19 = dat$PC19, pc20 = dat$PC20)
...
#Save output files 
write.table(assoc_out_sorted, "/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/EAS/GENESIS_assoc_tg_10_out.txt", row.names=F, quote=F)

#Create Manhattan and Q-Q plot 
png(file= "/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/EAS/GENESIS_tg_10_assoc_manhattan.png", width=8, height=7, units="in", res=300)
manhattan(assoc_out_sorted, chr="chr", bp="pos", snp="variant.id", p="Score.pval" )
dev.off()

png(file= "/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/EAS/GENESIS_tg_10_assoc_qq.png", width=8, height=7, units="in", res=300)
