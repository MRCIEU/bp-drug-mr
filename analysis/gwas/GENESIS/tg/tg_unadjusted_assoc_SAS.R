...
#Read in input data
phen_con <- read.table("/user/work/ac14629/MRC_network_project/data/UKB/phenotype/mrc_phenotypes_sas_int.txt", header=T, fill = T, na.strings = c("", NA), stringsAsFactors = FALSE)
...
#Save output files 
write.table(assoc_out_sorted, "/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/SAS/GENESIS_assoc_tg_out.txt", row.names=F, quote=F)

#Create Manhattan and Q-Q plot 
png(file= "/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/SAS/GENESIS_tg_assoc_manhattan.png", width=8, height=7, units="in", res=300)
manhattan(assoc_out_sorted, chr="chr", bp="pos", snp="variant.id", p="Score.pval" )
dev.off()

png(file= "/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/SAS/GENESIS_tg_assoc_qq.png", width=8, height=7, units="in", res=300)
