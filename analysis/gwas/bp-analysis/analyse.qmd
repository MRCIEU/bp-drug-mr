...
Not much signal beyond the European data.
...
Not much evidence for any heterogeneity. Plot the biggest heterogeneity SNPs (though none pass multiple testing)
...
The slopes are actually quite strong - plot
...
The degree of replication of the beta direction is in line with expectation across all populations

```{r}
lapply(eores, \(x) x$res) %>% bind_rows() %>% subset(metric=="P-value") %>% dplyr::select(pop, nsnp, datum, value, pdiff)
```

The degree of replication of the p-value is slightly higher than expected - this is because of ascertainment from the meta analysis.
