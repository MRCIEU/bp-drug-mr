...
extract <- lapply(pops, function(i) {
  cmd <- glue("grep -wf {tmp} /user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/ldl_analysis/stroke/GWAS_assoc_{i}_stroke_out.txt > {tmp}_{i}")
  system(cmd)
  fread(glue("{tmp}_{i}"))
})
...
a <- scan("/user/work/ac14629/MRC_network_project/results/UKB/GWAS/GENESIS/ldl_analysis/stroke/GWAS_assoc_afr_stroke_out.txt", what = "character", nlines = 1)
names(extract[[1]]) <- a
ex$afr <- extract[[1]] %>%
  as_tibble() %>%
...
saveRDS(ex, file = "/user/work/ac14629/MRC_network_project/results/ldl-analysis/pop_extract_ldlc_stroke_clumped.rds")
